...
contract operators instead of draining their funds; and b) contract operators to
...
we'd like to elaborate why it is difficult for operators to maintain their
...
cannot be changed in place [footnote 2]. The options for the contract operator are
...
implementation-specific, it likely defines an operator that is able to change the
...
contracts.  Secondly, we highlight the problems of contract operators suffering an
...
For a contract operator, the situation is equally tricky. (1) Bounty programs are
...
decisions taken by a contract operator under fire.  With regulation lagging
...
contract operator shut down the contract using a non-compliant procedure, then the
...
0xdeface.me serves as a hub of exchange for all stakeholders (operators, attackers
...
(public-key cryptography), the contract operator first has to share a public key
...
the contract operator's business anymore. (3) Shutting down and redeploying
contracts becomes a canonical process. (4) Vulnerabilities are disclosed
directly to the contract operator and do not end up on secondary markets instead.
...
users, attackers and contract operators. As we believe the gained utility to be of
...
#### 3. What if a contract operator decides to ignore a legit vulnerability?
...
It's of course in the contract operator's responsibility to make sure the damage
...
contract operator to decline the vulnerability with e.g. `string reason = "Damage
...
the game fairly. (3) Assume a contract operator implements a non-compliant shut
down procedure and an attacker submits a vulnerability anyway. If now the operator
...
operator is not playing the game fairly. We believe that in this case, the
contract operator's reputation would suffer significantly. To the point, in fact,
...
5. We acknowledge that setting `damage` to an incorrect value could potentially
   ...
   contract operator to do the math on the vulnerability. Is their decision to
